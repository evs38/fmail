	 ...
	 while ((!donePkt) && (!diskError) && (!breakPressed))
	 {
	    if ((ffblkPkt.ff_attrib & FA_RDONLY) == 0)
	    {
		  ...
		  if ((globVars.remoteCapability&1) == 1)
                     sprintf (tempStr, "           ภฤ %s %u.%02u, Type 2+, %uk, %u-%.3s-%u %u:%02u%s%s",
					...
					months+(globVars.month-1)*3, globVars.year,
                     ...
                     sprintf (tempStr, "           ภฤ %s, Type %s, %uk, %u-%.3s-%u %u:%02u%s%s",
					helpPtr,
		  ...
		  while ((!readPkt (message)) && (!diskError) && (!breakPressed))
		  {
			...
			writeMsg (message, PERMSG, 0);
		     ...
		     switch (areaIndex)
		     {
			case NETMSG : printString ("NETMAIL");
					 ...
					 else
					 {
				      ...
				      break;
			case BADMSG : printStringFill ("ออ Bad message");
...
tossbad:                              sprintf (tempStr, "\r\1FMAIL DEST: %s", nodeStr(&globVars.packetDestNode));
					 ...
					 insertLine (helpPtr, tempStr);
				      ...
				      globVars.badCount++;
				      break;
			default     : printString (echoAreaList[areaIndex].areaName);
							  ...
							  sizeof(nodeNumType)-2) != 0)))
					 {
					    ...
					    printStringFill (" Security violation ออ Bad message");
                                            goto tossbad;
					 }
				      }
		  ...
		  if (headerStat == 1)
		    ...
		    if (headerStat == 2)
		       logEntry ("Packet is addressed to another node ออ packet is renamed to .DST",
				 LOG_ALWAYS, 0);
		    else
		       if (headerStat == 3)
			  logEntry ("Packet password security violation ออ packet is renamed to .SEC",
				    LOG_ALWAYS, 0);
	    }
...
*/
	 ...
	 keyChecked = 2;
      ...
      {
	 case 1 : strcpy (stpcpy (tearline+4, config.tearLine), "\r");
		  break;
	 case 2 :
	 case 3 : break;
	 case 4 :
...
// struct tm      *timeBlock;
   ...
   printString (version);
#ifdef BETA
      printString (" ๙ The Fast Echomail Processor ๙ DO NOT DISTRIBUTE !");
#else
      printString (" ๙ The Fast Echomail Processor");
#endif
   gotoPos (3, 2);
...
/*    newLine (); */
   ...
   if (diskError)
   {
      newLine ();
      switch ( diskError )
      { case DERR_HACKER: logEntry("Internal overflow control error", LOG_ALWAYS, 1);
                          break;
        case DERR_VAL   : logEntry("File validate error", LOG_ALWAYS, 1);
                          break;
        case DERR_WRPKTE: logEntry("Echo PKT write error", LOG_ALWAYS, 1);
                          break;
        case DERR_CLPKTE: logEntry("Close PKT write error", LOG_ALWAYS, 1);
                          break;
        case DERR_PRSCN : logEntry("Scan handling error", LOG_ALWAYS, 1);
                          break;
        case DERR_PACK  : logEntry("Net message write error", LOG_ALWAYS, 1);
                          break;
        case DERR_WRHECHO:logEntry(MBNAME" echo write error", LOG_ALWAYS, 1);
                          break;
        case DERR_WRHBAD: logEntry(MBNAME" bad message write error", LOG_ALWAYS, 1);
                          break;
        case DERR_WRHDUP: logEntry(MBNAME" dup message write error", LOG_ALWAYS, 1);
                          break;
        case DERR_WRJECHO:logEntry("JAM echo write error", LOG_ALWAYS, 1);
                          break;
        default:          logEntry("Unknown disk error", LOG_ALWAYS, 1);
                          break;
      }
   }
   if (breakPressed)
...
#if 0
---------------------------------------------------------------------------
(9362)  Mon 5 Feb 96 12:41      Rcvd: Wed 7 Feb 21:04
By: grant beattie
To: Folkert Wijnstra
Re: OS/2 programming hint :)
St: Rcvd
---------------------------------------------------------------------------
Hello Folkert,
