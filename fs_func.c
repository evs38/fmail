...
s16 netmailMenu(u16 v)
{
   ...
   memcpy(&tempInfo.flagsRdRA    , &config.readFlagsRA[v], 4);
   memcpy(&tempInfo.flagsRdNotRA , &config.readFlagsRA[v][4], 4);
   tempInfo.writeSecRA          = config.writeSecRA[v];
   memcpy(&tempInfo.flagsWrRA    , &config.writeFlagsRA[v], 4);
   memcpy(&tempInfo.flagsWrNotRA , &config.writeFlagsRA[v][4], 4);
   tempInfo.sysopSecRA          = config.sysopSecRA[v];
   memcpy(&tempInfo.flagsSysRA   , &config.sysopFlagsRA[v], 4);
   memcpy(&tempInfo.flagsSysNotRA, &config.sysopFlagsRA[v][4], 4);
   tempInfo.templateSecQBBS     = config.templateSecQBBS[v];
   memcpy(&tempInfo.flagsTemplateQBBS, &config.templateFlagsQBBS[v], 4);
   ...
   memcpy(&config.readFlagsRA[v]      , &tempInfo.flagsRdRA        , 4);
   memcpy(&config.readFlagsRA[v][4]   , &tempInfo.flagsRdNotRA     , 4);
   config.writeSecRA[v]            = tempInfo.writeSecRA;
   memcpy(&config.writeFlagsRA[v]     , &tempInfo.flagsWrRA        , 4);
   memcpy(&config.writeFlagsRA[v][4]  , &tempInfo.flagsWrNotRA     , 4);
   config.sysopSecRA[v]            = tempInfo.sysopSecRA;
   memcpy(&config.sysopFlagsRA[v]     , &tempInfo.flagsSysRA       , 4);
   memcpy(&config.sysopFlagsRA[v][4]  , &tempInfo.flagsSysNotRA    , 4);
   config.templateSecQBBS[v]       = tempInfo.templateSecQBBS;
   memcpy(&config.templateFlagsQBBS[v], &tempInfo.flagsTemplateQBBS, 4);
