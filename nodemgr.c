...
{
  if (editType == EDIT_NODE || editType == DISPLAY_NODE_DATA || editType == DISPLAY_NODE_WINDOW)
    return code;
...
// addItem(nodeMenu, BOOL_INT                  , "Disabled"          ,  0, &tempInfoN.options         , BIT5, 0, "Set to fully disable a node");
   ...
   addItem(nodeMenu, DATETIME | DISPLAY        , "LastRcvd"          , 44, &tempInfoN.lastMsgRcvdDat  , 0                              ,            0, "Last time a message was received from this node"                                );
   ...
   addItem(nodeMenu, DATETIME | DISPLAY        , "LastSent"          , 44, &tempInfoN.lastMsgSentDat  , 0                              ,            0, "Last time a message was sent to this node"                                      );
   ...
   addItem(nodeMenu, DATETIME | DISPLAY        , "New bundle"        , 42, &tempInfoN.lastNewBundleDat, 0                              ,            0, "Last time a new bundle was created for this node"                               );
   addItem(nodeMenu, FUNC_PAR                  , "Groups"            ,  0, &groupsSelect              , 26                             ,            0, "Groups of message areas available to a node"                                    );
   addItem(nodeMenu, DATETIME | DISPLAY        , "Last BCL"          , 44, &tempInfoN.lastSentBCL     , 0                              ,            0, "Last time a BCL file was sent to this node"                                     );
   ...
   addItem(nodeMenu, ENUM_INT                  , "Netmail/flo status", 22, &statusToggle              , 0                              ,            6, "Status of generated netmails and flo files"                                     );
   addItem(nodeMenu, PATH                      , "File box"          ,  0,  tempInfoN.pktOutPath      , PKTOUT_PATH_LEN - 1            ,            0, "Directory where uncompressed PKT files will be stored, instead of normal export");
   ...
   addItem(nodeMenu, BOOL_INT                  , "Rem.maint       "  , 49, &tempInfoN.options         , BIT13                          ,            0, "If this node is allowed to perform remote maintenance"                          );
   ...
   addItem(nodeMenu, BOOL_INT                  , "Tiny SEEN-BYs   "  , 49, &tempInfoN.options         , BIT1                           ,            0, "Remove all SEEN-BYs except of your own downlinks (should normally NOT be used)" );
   ...
   addItem(nodeMenu, BOOL_INT                  , "Reformat date   "  , 49, &tempInfoN.options         , BIT0                           ,            0, "Correct the format of dates in forwarded messages (should normally NOT be used)");
   addItem(nodeMenu, BOOL_INT                  , "Pack netm."        ,  0, &tempInfoN.options         , BIT7                           ,            0,  "Pack netmail direct for this node"                                             );
   addItem(nodeMenu, NUM_INT                   , "Auto BCL days   "  , 49, &tempInfoN.autoBCL         , 3                              ,          999, "Interval in days between automatic BCL files"                                   );
   addItem(nodeMenu, WORD                      , "AreaMgr pwd"       ,  0, &tempInfoN.password        , sizeof(tempInfoN.password) - 2 ,            0, "Password for AreaMgr requests"                                                  );
   addItem(nodeMenu, NUM_INT                   , "AutoPassive days"  , 49, &tempInfoN.passiveDays     , 4                              ,         9999, "Max number of days between polls before auto-passive (1-9999, 0 = no max)"      );
   ...
   addItem(nodeMenu, NUM_INT                   , "AutoPassive size"  , 49, &tempInfoN.passiveSize     , 4                              ,         9999, "Max size of mail bundle before auto-passive (1-9999, 0 = no max)"               );
...
#define MENU_X_POS  1
   ...
   switch (editType)
   {
      case DISPLAY_NODE_WINDOW:
        update = displayMenu(nodeMenu, MENU_X_POS, MENU_Y_POS);
        break;
      case DISPLAY_NODE_DATA  :
        displayData(nodeMenu, MENU_X_POS, MENU_Y_POS, 0);
        update = 0;
        break;
      case EDIT_NODE          :
        update = runMenuD(nodeMenu, MENU_X_POS, MENU_Y_POS, NULL, setdef);
        break;
      case EDIT_NODE_GLOBAL   :
        runMenuD(nodeMenu, MENU_X_POS, MENU_Y_POS, NULL, setdef);
        update = 0;
        break;
   }
   if (setdef)
...
//---------------------------------------------------------------------------
void noMem(u16 selAreas, u16 selAreas2)
{
  while (selAreas)
...
//          printString ("\x1b\x1a", 77, 24, YELLOW, BLACK, MONO_HIGH);
         ...
         if ((nodeInfoCount != 0) || (ch == _K_INS_))
         {
            switch (ch)
            {
               ...
               case _K_F5_ :
               case _K_F9_ :
                  if ((count = nodesList(index)) != 0xffff)
                     index = count;
                  break;
               case _K_ENTER_ :
               case _K_F1_ :
                  ...
                  if ( openConfig(CFG_ECHOAREAS, &areaHeader, (void*)&areaBuf) )
                  {
                     ...
                     {  if ( getRec(CFG_ECHOAREAS, count) )
                        {
                           ...
                           {  if ( !memcmp(&tempInfoN.node, &areaBuf->forwards[count2++].nodeNum, sizeof(nodeNumType)) )
                                 break;
                           }
                     ...
                     {  if ( (listDataArray[--selAreas].flags & BIT15) &&
                        ...
                        {  count2 = 0;
                           ...
                           {  if ( !memcmp(&tempInfoN.node, &areaBuf->forwards[count2].nodeNum, sizeof(nodeNumType)) )
                                 break;
                              if ( areaBuf->forwards[count2].nodeNum.zone == 0 ||
                                   nodegreater(areaBuf->forwards[count2].nodeNum, tempInfoN.node) )
                              {  if ( count2 < MAX_FORWARD - 1 )
                                    memmove(&areaBuf->forwards[count2 + 1].nodeNum, &areaBuf->forwards[count2].nodeNum, (MAX_FORWARD - count2 - 1) * sizeof(nodeNumXType));
                                 areaBuf->forwards[count2].nodeNum = tempInfoN.node;
                                 break;
                              }
                              ++count2;
                           }
                     ...
                     {  if ( !(listDataArray2[--selAreas2].flags & BIT15) &&
                        ...
                        {  count2 = 0;
                           while ( count2 < MAX_FORWARD )
                           {  if ( !memcmp(&tempInfoN.node, &areaBuf->forwards[count2].nodeNum, sizeof(nodeNumType)) )
                              {  memmove(&areaBuf->forwards[count2].nodeNum, &areaBuf->forwards[count2 + 1].nodeNum, (MAX_FORWARD - count2 - 1) * sizeof(nodeNumXType));
                                 break;
                              }
                              ++count2;
                           }
               ...
               case _K_END_ :
                  ...
                  break;
               case _K_F6_:  // Copy node
                  if (nodeInfoCount == 0 || nodeInfoCount >= MAX_NODES)
                     break;
               ...
               case _K_INS_:
                  if (ch == _K_INS_)
                  {
                     // if continued from previous case don't set these
                     if (nodeInfoCount == MAX_NODES)
                        break; /* Insert */
                     memset(&tempInfoN, 0, sizeof(nodeInfoType));
                                 ...
                                 (askBoolean ("Delete this entry (also in export lists in the AreaMgr) ?", 'Y') == 'Y'))
                             {
                                ...
                                if (openConfig(CFG_ECHOAREAS, &areaHeader, (void*)&areaBuf))
                                {
                                   for (count = 0; count < areaHeader->totalRecords; count++)
                                   {
                                      ...
                                      while (count2 < MAX_FORWARD)
                                      {
                                         if (memcmp (&(areaBuf->forwards[count2].nodeNum),
                                                     &(nodeInfo[index]->node),
                                                     sizeof(nodeNumType)) == 0)
                                         {  memcpy (&(areaBuf->forwards[count2]),
                                                    &(areaBuf->forwards[count2+1]),
