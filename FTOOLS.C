...
//#if (!defined _Windows && !defined __OS2__) || defined __DPMI16__ || defined __DPMI32__
#ifdef __STDIO__
...
const char setBitTab[8] = {1, 2, 4, 8, 16, 32, 64, 128};

const char bitMaskTab[8] = {0, 1, 3, 7, 15, 31, 63, 127};

const char bitCountTab[256] =
...
//   u32            clusterSize;
   ...
   printString ("ีอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออธ\n");
   printString ("ณ                                                                             ณ\n");
   printString ("ณ                                                                             ณ\n");
   printString ("ิอออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออพ");
   setAttr (YELLOW, RED, MONO_HIGH);
   gotoPos (3, 1);
   printString (FTOOLS_VER_STRING);
#ifdef BETA
   printString (" ๙ The Fast Message Base Utility ๙ DO NOT DISTRIBUTE\n");
#else
   printString (" ๙ The Fast Message Base Utility\n");
#endif
   gotoPos (3, 2);
   printString ("Copyright (C) 1991-2008 by Folkert J. Wijnstra ๙ All rights reserved\n\n");
	gotoPos (0, 5);
...
/*
         getdfree (toupper(*config.bbsPath)-'A'+1, &dtable);
*/
//         clusterSize = diskClusterSize(config.bbsPath);
//         if (diskFree(config.bbsPath) <
//             ((newHdrSize*(s32)sizeof(msgHdrRec)+clusterSize-1)/clusterSize +
//              (newHdrSize*(s32)36+clusterSize-1)/clusterSize +
//              (newHdrSize*(s32)3+clusterSize-1)/clusterSize +
//              (newTxtSize*(s32)256+clusterSize-1)/clusterSize)*clusterSize)
         if ( diskFree(config.bbsPath) <
         ...
         for (count = 0; count < bufCount; count++)
         {
	       ...
	       if (msgHdrBuf[newBufCount].MsgAttr & RA_RECEIVED)
               ...
               if (switches & SW_P)
               {
                  ...
                  mask = bitMaskTab [d & 7];
                  d >>= 3;
                  msgHdrBuf[newBufCount].StartRec =
                                        keepTxt[d].usedCount +
                                        bitCountTab [keepTxt[d].used & mask];
               }
               newBufCount++;
	    }
      ...
      if (switches & SW_N)
      {
         ...
         if ((usersBBSHandle = open(expandName("USERS.BBS"), O_RDWR|O_BINARY|O_DENYNONE)) != -1)
         {
                 ...
                 config.bbsProgram == BBS_PROB || config.bbsProgram == BBS_ELEB )
            {
               ...
               {
                  if ((d=(u16)(filelength(usersBBSHandle)/1016))==0) d=1;
            ...
            else
            {
               ...
               {
                  if ((d=(u16)(filelength(usersBBSHandle)/158))==0) d=1;
...
//                            ((oldHdrSize*(s32)sizeof(msgHdrRec)+clusterSize-1)/clusterSize +
//                             (oldHdrSize*(s32)36+clusterSize-1)/clusterSize +
//                             (oldHdrSize*(s32)3+clusterSize-1)/clusterSize +
//                             (oldTxtSize*(s32)256+clusterSize-1)/clusterSize -
//                             (newHdrSize*(s32)sizeof(msgHdrRec)+clusterSize-1)/clusterSize -
//                             (newHdrSize*(s32)36+clusterSize-1)/clusterSize -
//                             (newHdrSize*(s32)3+clusterSize-1)/clusterSize -
//                             (newTxtSize*(s32)256+clusterSize-1)/clusterSize)*clusterSize);
      ...
      if ((switches & SW_A) == 0)
		{
         ...
         while (read(lastReadHandle, &lastReadRec, 400) == 400)
         {
            ...
            {
               if (maxRead < lastReadRec[count]) maxRead = lastReadRec[count];
            }
			}
         close (lastReadHandle);
         ...
         while ((msgIdxBufCount = (read(msgIdxHandle, msgIdxBuf, 512*sizeof(msgIdxRec))/3)) != 0)
         {
            ...
            {
               if (msgIdxBuf[count].MsgNum <= maxRead) index++;
            }
         }
         close (msgIdxHandle);
		}
...
//             else
         ...
         if (isNetmail && (stricmp (argv[count], "-aka") == 0))
	 {
      ...
      if ( switches & SW_F )
      {
//       if ( (lastReadHandle = open(message->subject, O_RDONLY|O_BINARY|O_DENYNONE)) == -1 )
...
//       close(lastReadHandle);
            ...
            if (switches & SW_N)
            {
               ...
               for (count = 0; count < bufCount; count++)
               {
                  if (count+1 != msgRenumBuf[count])
                  {
                     ...
                     {
                        gotoTab (0);
                        sprintf (tempStr, "- Renumbering messages: %u ออ %u",
                                           msgRenumBuf[count], count+1);
                        printString (tempStr);
                        updateCurrLine();
                        d++;
                     }
   ...
   if ((argc >= 2) &&
       ((stricmp(argv[1], "E") == 0) || (stricmp(argv[1], "SendSMTP") == 0)))
   {  if ( argc >= 3 && (argv[2][0] == '?' || argv[2][1] == '?') )
      {  printString ("Usage:\n\n"
                      "    FTools SendSMTP\n");
         showCursor ();
   ...
   {
      printString ("Usage:   FTools <command> [parameters]\n\n"
                   "Commands:\n\n"
#ifndef GOLDBASE
...
#if 0
                   "   SendSMTP  Sends mail through the Internet\n"
#endif
