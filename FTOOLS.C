...
//   u32            clusterSize;
...
#ifdef BETA
  printString (" ù The Fast Message Base Utility ù DO NOT DISTRIBUTE\n");
#else
       ...
       (stricmp(argv[1], "V") == 0) || (stricmp(argv[1], "MOVE") == 0)))
  {
    ...
    if ((argc >= 3) && ((argv[2][0] == '?') || (argv[2][1] == '?')))
    {
      ...
      else if (switches & SW_U)
      {
        printString ("Usage:\n\n"
                     "    FTools Undelete <board>\n\n");
      }
      else if (switches & SW_M)
      {
        printString ("Usage:\n\n"
                              ...
                              sizeof(msgHdrRec)) / sizeof(msgHdrRec)) > 0)
    {
      ...
      for (count = 0; count < bufCount; count++)
      {
        ...
        if ((!(switches & SW_P)) || (keepHdr[keepIdx] & keepBit))
        {
          ...
          else if (msgHdrBuf[newBufCount].MsgAttr & RA_RECEIVED)
    ...
    */
             ...
             ((stricmp(argv[1], "P") == 0) || (stricmp(argv[1], "POST") == 0)))
    {
      ...
      for (count = 4; count < argc; count++)
      {
        ...
        else if (stricmp (argv[count], "-to") == 0)
        {
          if ( count+1 < argc && *argv[count+1] != '-' )
        ...
        else if (stricmp (argv[count], "-subj") == 0)
        {
          if ( count+1 < argc && *argv[count+1] != '-' )
          {
//             if (switches & SW_F)
...
//             else
        ...
        else if (isNetmail && (stricmp (argv[count], "-aka") == 0))
        {
          if ( count+1 < argc && *argv[count+1] != '-' &&
        ...
        else if (isNetmail && (stricmp (argv[count], "-dest") == 0))
        {
          if ( count+1 < argc && *argv[count+1] != '-' )
          {
            temp = 0;
...
//       close(lastReadHandle);
             ...
             ((stricmp(argv[1], "G") == 0) || (stricmp(argv[1], "MSGM") == 0)))
    {
      ...
      for (c = 2; c < argc; c++)
      {
        ...
        else if (stricmp(argv[c], "-pmail") == 0)
        {
          ...
          if (switches & SW_N)
          {
            ...
            for (count = 0; count < bufCount; count++)
            {
              if (count+1 != msgRenumBuf[count])
              {
                ...
                {
                  gotoTab (0);
                  sprintf (tempStr, "- Renumbering messages: %u ï¿½ï¿½ %u",
