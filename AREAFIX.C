...
typedef struct
{
   ...
   signed char uplink ;
   s16         maxRescan;
   char        *areaName;   }  areaFixType;

typedef struct
{
   s16  index;
   char groupChar; } areaSortType;
   ...
   if (nodeInfoPtr->options.forwardReq)
   {
      ...
      do
      {
             ...
             ((nodeInfoPtr->groups & config.uplinkReq[count].groups) != 0))
         {
            ...
            if ( !*config.uplinkReq[count].fileName )
               continue;
   ...
   for (count = 0; count < 2; count++)
   {
          ...
          ((count == 1) && (nodeInfoPtr2 != NULL)))
      {
	    ...
	    node2d (&message->destNode);
         ...
         {
            case 1 : message->attribute |= HOLDPICKUP;
                     break;
            case 2 : message->attribute |= CRASH;
                     break;
            case 3 : message->attribute |= HOLDPICKUP;
                     helpPtr = insertLine (helpPtr, "\1FLAGS DIR\r");
                     break;
            case 4 : message->attribute |= CRASH;
	    case 5 : helpPtr = insertLine (helpPtr, "\1FLAGS DIR\r");
                     break;
         }
	 ...
	 allType = 0;
		 ...
		 /* Skip kludge lines and comment lines */
	    {
		  ...
		  helpPtr++;
                  ...
                  if (config.mgrOptions.allowPassword &&
			...
			passwordChange = 1;
                  ...
                  if (config.mgrOptions.allowPktPwd &&
                  ...
                  if (config.mgrOptions.allowBCL &&
                      strnicmp(helpPtr, "AUTOBCL ", 8) == 0)
                  {
                     nodeInfoPtr->autoBCL = atoi(helpPtr + 8);
                     autoBCLchange = 1;
                  } else
                  if ((strnicmp(helpPtr, "HELP", 4) == 0) && !isalnum(*(helpPtr+4)))
		  {
                     replyHelp = 1;
                  } else
                  if ( (strnicmp(helpPtr, "LIST", 4) == 0) &&
                  ...
                  if ((strnicmp(helpPtr, "BLIST", 5) == 0) && !isalnum(*(helpPtr+5)))
                  {
                     replyBCL = 1;
                  } else
                  if ((strnicmp(helpPtr, "QUERY", 5) == 0) && !isalnum(*(helpPtr+5)))
                  {
                     replyQuery = 1;
                  } else
                  if ((strnicmp(helpPtr, "UNLINKED", 8) == 0) && !isalnum(*(helpPtr+8)))
                  {
                     replyUnlinked = 1;
                  } else
                  if ((strnicmp(helpPtr, "RESCAN", 6) == 0) && !isalnum(*(helpPtr+6)))
                  {
                     rescanRequest++;
			...
			rescanAll = config.defMaxRescan ?
                      ...
                      (strnicmp(helpPtr, "NOTIFY ", 7) == 0))
                  {
                     if ((strnicmp (helpPtr+7, "ON", 2) == 0) && !isalnum(*(helpPtr+9)))
                        ...
                        if ((strnicmp (helpPtr+7, "OFF", 3) == 0) && !isalnum(*(helpPtr+10)))
                           notifyChange = 2;
                  } else
                  if (config.mgrOptions.allowAddAll &&
			   ...
			   rescanAll = config.defMaxRescan ?
                  ...
                  if ((strnicmp(helpPtr, "-ALL", 4) == 0) && !isalnum(*(helpPtr+4)))
                  {
		               areaFixCount = 0;
                     allType = DELETE_ALL;
                  }
                  else
                  if (strnicmp(helpPtr, "FROM", 4) != 0)
                  {
...
//       availCount = 0;
	 ...
	 for (count = 0; count < areaFixCount; count++)
	 {
            ...
            switch ((*areaFixList)[count].remove)
	    {
			...
			break;
	       case 3 : helpPtr += sprintf (helpPtr+43, " already active")+43;
			break;
               case 4 : helpPtr += sprintf (helpPtr+43, " removed")+43;
                        break;
	       case 5 : helpPtr += sprintf (helpPtr+43, " not active")+43;
			break;
               case 1 :
               case 6 : helpPtr += sprintf (helpPtr+43, " not found")+43;
                        break;
               case 7 : helpPtr += sprintf (helpPtr+43, " not allowed")+43;
                        break;
               case 8 : helpPtr += sprintf (helpPtr+43, " rescan requested")+43;
                        break;
               case 9 : helpPtr += sprintf (helpPtr+43, " rescan not allowed")+43;
                        break;
               default: if (((*areaFixList)[count].uplink =
