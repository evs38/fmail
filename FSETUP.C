...
// addItem (netMenu, DISPLAY, NULL, 0, NULL, 0, 0, NULL);
   ...
   addItem (mbMenu, BOOL_INT, "ÈÍ Use subject", 0, &config.mbOptions, BIT1, 0,
...
*/
   ...
   addItem (genMenu, TEXT, "ÈÍ Custom {+}", 0, &config.tearLine, 24, 0,
...
/*   addItem (mainMenu, NEW_WINDOW, "AKA matching", 0, akaMatchMenu, 2, -2,
   ...
   if (ch == 'Y')
   {
      ...
      {  if ( !stricmp(config.autoFMail102Path, configPath) )
         ...
         if ( (configHandle = open(strcat(strcpy(tempStr, config.autoFMail102Path), "FMAIL.CFG"), O_WRONLY|O_BINARY|O_CREAT|O_TRUNC, S_IREAD|S_IWRITE)) != -1 )
