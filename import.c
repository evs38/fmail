...
static u16 checkDupBoard(u8 *echoNumCheck, rawEchoType *echoAreaRec, u16 *echoCount, u16 *clearboard)
{
  ...
  if (echoAreaRec->board > 0 && echoAreaRec->board <= MBBOARDS && echoNumCheck[echoAreaRec->board - 1]++ != 0)
  {
   ...
   {  if ( !(fdAreaFileRec.deleted || fdAreaFileRec.tsAreaFlags.deleted) )
      {
         ...
         strncpy(echoAreaRec->areaName, fdAreaFileRec.areaName, ECHONAME_LEN - 1);
...
s16 importGechoAr(void)
{
   ...
   {  if ( !(gechoArRec.options & REMOVED) )
      {  if ( echoCount == MAX_AREAS )
         ...
         {
            tempNode.zone  = gechoArExportRec[count].address.zone;
            tempNode.net   = gechoArExportRec[count].address.net;
            tempNode.node  = gechoArExportRec[count].address.node;
            tempNode.point = gechoArExportRec[count].address.point;
            node4d(&tempNode);
            low = 0;
            high = count;
            while ( low < high )
...
ret0:    releaseNodes(nodeCount);
      ...
      nodeInfoPtr->node.zone  = gechoNdRec.address.zone;
      nodeInfoPtr->node.net   = gechoNdRec.address.net;
      nodeInfoPtr->node.node  = gechoNdRec.address.node;
      nodeInfoPtr->node.point = gechoNdRec.address.point;
      node4d(&nodeInfoPtr->node);
...
ret0:    releaseNodes(nodeCount);
      ...
      if ( (nodeInfoPtr = malloc(sizeof(nodeInfoType))) == NULL )
         goto ret0;
      memset(nodeInfoPtr, 0, sizeof(nodeInfoType));
      nodeInfoPtr->node.zone  = nodeFE.addr.zone;
      nodeInfoPtr->node.net   = nodeFE.addr.net;
      nodeInfoPtr->node.node  = nodeFE.addr.node;
      nodeInfoPtr->node.point = nodeFE.addr.point;
      node4d(&nodeInfoPtr->node);
   ...
   {  working();
      ...
      {  if ( areaBuf->board >= 1  &&  areaBuf->board <= MBBOARDS )
         {  lseek(RAHandle, (areaBuf->board-1)*(s32)sizeof(messageRaType), SEEK_SET);
            ...
            memcpy(&areaBuf->flagsRdRA , &messageRaRec.readFlags , 4);
            memcpy(&areaBuf->flagsWrRA , &messageRaRec.writeFlags, 4);
            memcpy(&areaBuf->flagsSysRA, &messageRaRec.sysopFlags, 4);
            putRec(CFG_ECHOAREAS, count);
            echoCount++;
         }
         count++;
...
// NIEUW:        (areaBuf->board == BOARDTYPEJAM &&
            ...
            {  notFound = 0;
               ...
               memcpy(&areaBuf->flagsRdRA    , &messageRa2Rec.readFlags    , 4);
               memcpy(&areaBuf->flagsRdNotRA , &messageRa2Rec.readNotFlags , 4);
               memcpy(&areaBuf->flagsWrRA    , &messageRa2Rec.writeFlags   , 4);
               memcpy(&areaBuf->flagsWrNotRA , &messageRa2Rec.writeNotFlags, 4);
               memcpy(&areaBuf->flagsSysRA   , &messageRa2Rec.sysopFlags   , 4);
               memcpy(&areaBuf->flagsSysNotRA, &messageRa2Rec.sysopNotFlags, 4);
               if ( setBoardCode )
