...
void addNew(s32 switches)
{
   ...
   {  badEchoCount = (read(tempHandle, badEchos,
	 ...
	 index = 0;
	 while ( index < areaInfoCount &&
		 strcmp(tempInfo.areaName, (*areaNames)[index]) > 0 )
	 {  index++;
	 }
	 if ( index < areaInfoCount &&
	      !strcmp(tempInfo.areaName, (*areaNames)[index]) )
	 {  ++areasExist;
	 ...
	 if ( index < areaInfoCount++ )
	 {  memmove(&(*areaNames)[index+1], &(*areaNames)[index],
		    (areaInfoCount-index) * sizeof(char *));
	    memmove(&(*mbPaths)[index+1], &(*mbPaths)[index],
		    (areaInfoCount-index) * sizeof(char *));
	 }
