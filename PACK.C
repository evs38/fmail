...
void packRoute (char *packedNodes, char *exceptNodes,
                char destType, nodeNumType *destNode, s32 switches)
{
   u16         count;
   u16         oldAttribute;
...
/*
   tempStrType tempStr;
   sprintf (tempStr, "Nodes >%s<\nexcept >%s<\nvia >%s<\ntype %u\n", packedNodes, exceptNodes, nodeStr(destNode), destType);
   printString(tempStr);
   newLine();
*/
   for (count = 0; count < netIndex; count++)
   {
      ...
      switch (destType)
      {
	 case DEST_NONE : memcpy (&currLineDestNode,
                                  &(*netList)[count].destNode,
                                  sizeof(nodeNumType));
                          break;
         case DEST_VIA  : memcpy (&currLineDestNode, destNode,
                                  sizeof(nodeNumType));
                          break;
         case DEST_HOST : memcpy (&currLineDestNode,
                                  &(*netList)[count].destNode,
                                  sizeof(nodeNumType));
      ...
      {
         memcpy (&(*netList)[count].viaNode,
                 &currLineDestNode, sizeof(nodeNumType));
      }
   }
}
      ...
      if ((helpPtr = strstr (lineBuf, " /")) != NULL)
      {
         *helpPtr = 0;
         ...
         while (helpPtr != NULL)
         {
            ...
            if ((helpPtr[0]!='/') ||
                ((helpPtr[1]!='I') && (helpPtr[1]!='L') &&
                 (helpPtr[1]!='C') && (helpPtr[1]!='H') && (helpPtr[1]!='O')) ||
                (helpPtr[2]!=0))
            {
...
s16 pack (s16 argc, char *argv[], s32 switches)
{
   ...
   while ( !doneMsg )
   {
          ...
          !(message->attribute & FILE_ATT))
      {
         ...
         if (count == MAX_AKAS)
         {
            ...
            memmove (&((*netList)[low+1]), &((*netList)[low]),
                     (netIndex++-low) * sizeof(netRecType));
   ...
   {
      if (!(*netList)[index].viaNode.zone) continue;
      ...
      {
         if (memcmp(&(*netList)[count].viaNode, &tempNode,
