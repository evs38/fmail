...
// for Win16 compile
	...
	if (dosExitCode == -1)
	{
		...
		switch (errno)
		{
			case E2BIG   : strcat (tempStr, "Arg list too big");
			break;
	 case EINVAL  : strcat (tempStr, "Invalid argument");
			break;
	 case ENOENT  : strcat (tempStr, "Path or file name not found");
			break;
	 case ENOEXEC : strcat (tempStr, "Exec format error");
			break;
	 case ENOMEM  : strcat (tempStr, "Not enough memory");
			break;
		}
		logEntry (tempStr, LOG_ALWAYS, 0);
		return (1);
   ...
   else
   {
	 ...
	 *(helpPtr-1) = '.';
	 ...
	 if (config.mailOptions.createSema)
	 {
      ...
      else if ( config.mailer == 3 || config.mailer == 5 )
      {
...
//          nodeInfo->lastNewBundleDat = nodeInfo->referenceLNBDat = startTime;
   ...
   switch (archiver)
   {
      ...
      case 10 : /* JAR */
                ...
                extPtr = "JAR";
                break;
      default : sprintf(tempStr, "Unknown archiving utility listed for node %s", nodeStr(destNode));
                logEntry(tempStr, LOG_ALWAYS, 0);
      ...
      if (fileAttach (archiveStr, srcNode, destNode, nodeInfo))
	 {
      ...
      else
      {
         ...
         {   sprintf (archivePtr, ".%cLO", (nodeInfo->outStatus==1)?'H':
                                           ...
                                           ((nodeInfo->outStatus>=3)&&(nodeInfo->outStatus<=5))?'D':'F');
         }
         else
