...
/* Compile with FTOOLS defined and rename to FTSORTHB.OBJ for FTOOLS */
/* #define FTOOLS 1 */
...
/*   struct dfree dtable; */
  ...
  u16           *sli_subjectCrcHi,
   ...
   if ((msgHdrHandle = openP(expandName("MSGHDR", 0), O_RDONLY|O_BINARY|O_DENYNONE, S_IREAD|S_IWRITE)) == -1)
   {
...
/*                                 dtable.df_avail*(s32)dtable.df_bsec*
   ...
   if (config.mbOptions.sortNew)
   {
      ...
      if (((msgIdxHandle = openP(expandName("MSGIDX", 0),
                                 O_WRONLY|O_CREAT|O_TRUNC|O_BINARY|O_DENYNONE,
                                 S_IREAD|S_IWRITE)) == -1) ||
          ((msgToIdxHandle = openP(expandName("MSGTOIDX", 0),
                                   O_WRONLY|O_CREAT|O_TRUNC|O_BINARY|O_DENYNONE,
                                   S_IREAD|S_IWRITE)) == -1))
      {
         close (msgIdxHandle);
      ...
      if (((unlink (tempStr2) == -1) && (errno != ENOENT)) ||
          ((msgHdrHandle = openP(tempStr3, O_WRONLY|O_CREAT|O_TRUNC|O_BINARY|O_DENYNONE,
                                           S_IREAD|S_IWRITE)) == -1) ||
          ((oldHdrHandle = openP(tempStr1, O_RDONLY|O_BINARY|O_DENYNONE, S_IREAD|S_IWRITE)) == -1))
      {
         close (msgHdrHandle);
         logEntry ("Can't create sorted/linked message files", LOG_ALWAYS, 0);
         goto exit1;
   ...
   else
   {
      ...
      if (((msgHdrHandle = openP(tempStr1, O_WRONLY|O_BINARY|O_CREAT|O_DENYNONE,
                                           S_IREAD|S_IWRITE)) == -1) ||
          ((oldHdrHandle = openP(tempStr1, O_RDONLY|O_BINARY|O_DENYNONE,S_IREAD|S_IWRITE)) == -1))
      {
         close (msgHdrHandle);
