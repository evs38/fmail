...
void openDup(void)
{
   ...
   if ( ((dupHandle = openP(tempPath, O_RDONLY|O_DENYNONE|O_BINARY, S_IREAD|S_IWRITE)) == -1) ||
...
void closeDup(void)
{
   ...
   for (group = 0; group < 256; group++)
   {
      if ( (validDup = nextDupOld[group]) != (currentDup = dupHdr.nextDup[group]) )
      {
         ...
         {  memset(&dupBuffer[dupHdr.kRecs*4*group + validDup],
                   0, (currentDup-validDup)*4);
         }
         else
         {   memset(&dupBuffer[dupHdr.kRecs*4*group], 0, currentDup*4);
             memset(&dupBuffer[dupHdr.kRecs*4*group + validDup],
                    0, (dupHdr.kRecs*4-validDup)*4);
         }
      }
   }
