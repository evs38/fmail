...
void moveMsg (char *msgName, char *destDir)
{
   ...
   if (*destDir)
   {
      if (destDir == config.sentPath) highMsgNum = highMsgNumSent;
         ...
         if (destDir == config.rcvdPath) highMsgNum = highMsgNumRcvd;
      ...
      if (destDir == config.sentPath) highMsgNumSent = highMsgNum;
         ...
         if (destDir == config.rcvdPath) highMsgNumRcvd = highMsgNum;
   }
}
...
s32 writeMsg (internalMsgType *message, s16 msgType, s16 valid)
{
   ...
   */
   ...
   switch (msgType)
   {
      case NETMSG:   strcpy (tempStr, config.netPath);
		     break;
      case PERMSG:   strcpy (tempStr, config.pmailPath);
		     break;
      case SECHOMSG: strcpy (tempStr, config.sentEchoPath);
		     break;
      default      : return (-1);
   }
   ...
   /* if (!valid) */
      {
         switch (msgType)
         {
            case NETMSG: globVars.netCount++;
                         globVars.createSemaphore++;
                         break;
            case PERMSG: globVars.perCount++;
                         break;
            default:     return (-1);
         }
      }
...
void validateMsg (void)
{
   ...
   for (c = PERMSG; c <= NETMSG; c++)
   {
          ...
          ((c == PERMSG) && (globVars.perCount)))
      {
         switch (c)
         {
            case NETMSG : strcpy (tempStr1, config.netPath);
                          break;
            case PERMSG : strcpy (tempStr1, config.pmailPath);
                          break;
         }
                ...
                nodeInfoType *nodeInfo)
{
   ...
   switch (nodeInfo->outStatus)
   {
      ...
      case 5 : strcat (message->text, " DIR");
               break;
   }
